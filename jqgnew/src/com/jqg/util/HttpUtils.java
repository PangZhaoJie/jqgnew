...
public class HttpUtils {
	...
	public static List URLGet(String strUrl, Map map) throws IOException {
		...
		HttpURLConnection con = getConnection(strtTotalURL);
	...
	private static HttpURLConnection getConnection(String strtTotalURL) throws IOException {
		URL url = new URL(strtTotalURL);
		HttpURLConnection con = (HttpURLConnection) url.openConnection();
		con.setUseCaches(false);
		con.setFollowRedirects(true);
		if (con.HTTP_MOVED_PERM == con.getResponseCode()) {
			String redictURL = con.getHeaderField("Location");
			log.info("one more time:" + redictURL);
			return getConnection(redictURL);
		}
		return con;
	}
