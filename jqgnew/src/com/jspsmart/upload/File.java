 ...
 public class File
 {
     ...
     throws ServletException, IOException, SmartUploadException, SQLException
   {
     ...
     l = BigInteger.valueOf(this.m_size).divide(BigInteger.valueOf(i))
       .longValue();
     ...
     try {
       for (int i1 = 1; i1 < l; i1++) {
         resultset.updateBinaryStream(s, new ByteArrayInputStream(
           this.m_parent.m_binArray, k, i), i);
         k = k != 0 ? k : 1;
       ...
       if (j > 0)
         resultset.updateBinaryStream(s, new ByteArrayInputStream(
           this.m_parent.m_binArray, k, j), j);
     } catch (SQLException sqlexception) {
       byte[] abyte0 = new byte[this.m_size];
       System.arraycopy(this.m_parent.m_binArray, this.m_startData, abyte0, 0, 
         this.m_size);
