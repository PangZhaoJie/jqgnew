...
<div class="maincontent">
  <div class="conbox pt20">
    <P class="guide">当前位置：<a href="<%=path%>/index.action">网站首页</a> > <a href="<%=path%>/tender/totenderList?lussingType=9">企业直投</a></P>
...
function getJSONData(currentPage,pageSize) {
	...
	var url = "tender/lssuinglistPage.action?currentPage="+currentPage+"&lussingType=9&pageSize="+pageSize+"&satate=" + 2+"&temp=" +Math.random();
	 jQuery.getJSON(url,function(data){
		...
		for(var i = 0; i < jsonRoot.length; i++) {
			str += '<tr><td style="text-align: left; "><a href="' + decodeURI(jsonRoot[i].actions, "utf-8") + '&lussingType=9" ><img src="../images/loantype/firm.gif" />'+ jsonRoot[i].picture+ jsonRoot[i].title + "</a></td><td>"
				 ...
				 + decodeURI(jsonRoot[i].bear, "utf-8") + '%;" class="jdt_cont"></span></div></div></td><td> <a href="' + decodeURI(jsonRoot[i].action, "utf-8") + '&lussingType=9" class="' + decodeURI(jsonRoot[i].stataclass , "utf-8") +'">'
...
function getJSONData2(currentPage,pageSize) {
	 document.getElementById("pageroots").style.display="none";
	var url = "tender/lssuinglistPage.action?currentPage="+currentPage+"&lussingType=9&pageSize="+pageSize+"&satate=" + 3+"&temp=" +Math.random();
	 jQuery.getJSON(url,function(data){
		var cp = data.currentPage;
		var tp = data.totalPage;
...
function getJSONData3(currentPage,pageSize) {
	 document.getElementById("pageroots").style.display="none";
	var url = "tender/lssuinglistPage.action?currentPage="+currentPage+"&lussingType=9&pageSize="+pageSize+"&satate=" + 4+"&temp=" +Math.random();
	 jQuery.getJSON(url,function(data){
		var cp = data.currentPage;
		var tp = data.totalPage;
 ...
 function getJSONDatas(currentPage,pageSize) {
    ...
    if(parseInt(jQuery("#endmoney").val()) < parseInt(jQuery("#startMoney").val())){
	 ...
	 if(document.getElementById("dayCheckId").checked){
			url = "tender/findlssuingBytender.action?currentPage="+currentPage+"&lussingType=9&pageSize="+pageSize+"&returnWayId="+ jQuery("#time0").val()+"&moneyUseId="+ jQuery("#yongtu").val()+"&periodTimeId="
