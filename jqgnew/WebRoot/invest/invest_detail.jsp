...
<div class="maincontent">
    ...
    <input type="text"  style="display: none;" value="<fmt:formatNumber value="${sessionScope.AvailableMoney}" pattern="0.00"/> "  id="moneyIDnone" ></input>
    ...
    <div class="conbox pt20">
     ...
     <div class="tend_nr fn_cle">
       ...
       <div class="tend_info fn_cle">
          <div class="tend_l">
            <ul>
<!--            <li>借款金额：<span>${sessionScope.lssuing.borrowMoney}</span>元 </li>                   -->
    ...
    <div class="tend_r">
      <div class="tend_form">
       <form action="<%=path%>/json/tender" method="post" name="tender" onsubmit="getElementById('invest_bt').disabled=true;return true;">
       <input type="text"  style="display: none;" value="${sessionScope.lssuing.lssuingId}"  id="lssuingId" name="lssuingId"></input>
            ...
            <a href="<%=path %>/user/login?#yanzhengzhongxin" style="color:red"> [马上验证]</a></div></s:if>
            ...
            <div id="textState">
               ...
               <em id="moneyID" style="display: none;color: red;" >对不起余额不足<a href="<%=path%>/borrow/toChongzhi?userId=${session.uservip.userId}" style="color: blue">[请充值]</a></em>
               ...
               <div id="minmaxState"><label >最低投标金额：<fmt:formatNumber value="${sessionScope.moneyMin}" pattern="0.00"/>元 最高投标金额：<c:if  test="${sessionScope.moneyMax==0}"  >无限制</c:if><c:if  test="${sessionScope.moneyMax !=0}"  >${sessionScope.moneyMax}</c:if></label></div>
             ...
             <div class="invest_btn"  >
                   ...
                   <input type="button" class="invest_bt" id="5State" value="已流标" style="display: none;"/>
             </div>
             </form>  
             </div>
            </div>
      ...
      <div id="myCont2">
         <div class="current" id="list_3">
            <div class="personal_infor">
              <div class="tend_left">
                  <div class="cle"></div>
                  <div class="mt30" style="margin-top: 0px;">
                  <c:if test="${not empty uservip.userId}">
                      <h3>借款详细介绍</h3>
                       ...
                       <h3>担保详情</h3>
                        ...
                        <div class="tend_con">由推荐方天印融资租赁（上海）有限公司不可撤销地承诺对其推荐的融资项目承担连带保证责任。如融资人不能按期及时履行还款义务或回购承诺人未履行回购义务的，则推荐方将于债务到期日，按约定清偿投资人持有的尚未履行的全部债权。项目推荐方为确保承诺顺利实现，向招米贷平台缴纳一定比例的保证金，并授权招米贷于债务到期日在未能收到回购承诺人回购价款的基础上启动保证金代为清偿债务。保证金制度是补充、优化回购方回购能力的有效手段。在回购承诺人出现回购不及时情形下，招米贷可第一时间启动保证金清偿债务，代回购承诺人向投资人履行回购义务，推荐方并在约定时间内完成资金补足。平台将启动法律援助，招米贷提取一定额度的自有资金设立法律援助基金，该笔资金专项用于为招米贷平台的全部投资人提供法律援助支持。如任一融资项目发生逾期并须启动法律程序时，招米贷将启用该笔资金为投资人聘请并支付专业律师为投资人启动法律程序解决争议，确保投资人在极端情况下能够及时行动有效维权，最大限度地保护投资人的利益。</div>
                        </c:if>
