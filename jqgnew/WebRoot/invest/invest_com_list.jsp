...
<div class="maincontent">
  <div class="conbox pt20">
    <P class="guide">当前位置：<a href="<%=path%>/index.action">网站首页</a> > <a href="<%=path%>/tender/totenderList">我要投资</a></P>
          ...
          <div class="choose1"><h4>项目状态</h4> 
                 <div id="uboxstyle">
                    <select name="bid" id="bid">
	              	  ...
	              	  <option value="0">初审中的借款</option>
		              <option value="2">正在投资借款</option>
		              <option value="3">还款中的借款</option>
		              <option value="4">已完成的借款</option>
		            </select>
                  </div>
       ...
       <div id="hotnews_caption">
         <ul id="myTop1">                  
            ...
            <li class="normal" onclick="secBoard('hotnews_caption','list',2);" >即将上线</li>
 			<li class="normal" onclick="secBoard('hotnews_caption','list',5);">复审中的借款</li>
            <li class="normal" onclick="secBoard('hotnews_caption','list',3);">正在回款</li>
            <li class="normal" onclick="secBoard('hotnews_caption','list',4);">募集成功</li>
         </ul>
      </div>
...
<div id="list_5"  class="normal">
 <div class="list_4_table">

     <table width="1140" border="0" align="center" cellspacing="0" cellpadding="0" class="tbjl_table" id="listTable5">
  		<tr  class="header">	          	          	              	  	        
    		<td  style="width:250px;">项目名称</td><td>信用等级</td><td>当前利率</td> <td>投标奖励</td><td>借款总额</td><td>借款期限</td><td>进度</td><td>状态</td>
  		</tr>
  	</table>
 </div>
  <div id="pageroot5" class="page" style="display: none;">
...
function getJSONData(currentPage,pageSize) {
	...
	var url = "tender/lssuinglistPage.action?currentPage="+currentPage+"&pageSize="+pageSize+"&satate=" + 2+"&temp=" +Math.random();
	 jQuery.getJSON(url,function(data){
		...
		for(var i = 0; i < jsonRoot.length; i++) {
			str += '<tr><td style="text-align: left; "><a href="' + decodeURI(jsonRoot[i].actions, "utf-8") + '" >'+ jsonRoot[i].picture+ jsonRoot[i].title + "</a></td><td>"
				 ...
				 + decodeURI(jsonRoot[i].bear, "utf-8") + '%;" class="jdt_cont"></span></div></div></td><td> <a href="' + decodeURI(jsonRoot[i].action, "utf-8") + '" class="' + decodeURI(jsonRoot[i].stataclass , "utf-8") +'">'
...
function getJSONData1(currentPage,pageSize) {
	 document.getElementById("pageroots").style.display="none";
	var url = "tender/lssuinglistPage.action?currentPage="+currentPage+"&pageSize="+pageSize+"&satate=" + 0+"&temp=" +Math.random();
	 jQuery.getJSON(url,function(data){
		var cp = data.currentPage;
		var tp = data.totalPage;
		var  $phpernotetable =  jQuery("#listTable1");
		 $phpernotetable.empty();
		str = '<tr class="header">  <td  >项目名称</td><td style="width:140px;" >当前利率</td> <td>投标奖励</td><td>借款总额</td><td>借款期限</td><td>进度</td><td>状态</td></tr>';
		var jsonRoot = data.jsonRoot;
		if(tp !=1&&tp !=0)
		{
    	 ...
    	 document.getElementById("pageroot1").style.display="";
		}
		for(var i = 0; i < jsonRoot.length; i++) {
			str += '<tr><td style="text-align: left; "><a href="' + decodeURI(jsonRoot[i].actions, "utf-8") + '" >'+ jsonRoot[i].picture+ jsonRoot[i].title + "</a></td><td>"
				   + decodeURI(jsonRoot[i].rate, "utf-8") + "%</td><td>"
		 ...
		 jQuery("#firstPage1").attr("href","javascript:getJSONData1(1,10)");
		 jQuery("#prevPage1").attr("href","javascript:getJSONData1("+(parseInt(cp)-1)+",10)");
		 jQuery("#nextPage1").attr("href","javascript:getJSONData1("+(parseInt(cp)+1)+",10)");
		 jQuery("#lastPage1").attr("href","javascript:getJSONData1("+(parseInt(tp))+",10)");
	});
}
function getJSONData2(currentPage,pageSize) {
	 document.getElementById("pageroots").style.display="none";
	var url = "tender/lssuinglistPage.action?currentPage="+currentPage+"&pageSize="+pageSize+"&satate=" + 3+"&temp=" +Math.random();
	 jQuery.getJSON(url,function(data){
		var cp = data.currentPage;
		var tp = data.totalPage;
...
function getJSONData3(currentPage,pageSize) {
	 document.getElementById("pageroots").style.display="none";
	var url = "tender/lssuinglistPage.action?currentPage="+currentPage+"&pageSize="+pageSize+"&satate=" + 4+"&temp=" +Math.random();
	 jQuery.getJSON(url,function(data){
		var cp = data.currentPage;
		var tp = data.totalPage;
...
function getJSONData5(currentPage,pageSize) {
	 document.getElementById("pageroots").style.display="none";
	var url = "tender/lssuinglistPage.action?currentPage="+currentPage+"&pageSize="+pageSize+"&satate=" + 1+"&temp=" +Math.random();
	 jQuery.getJSON(url,function(data){
		var cp = data.currentPage;
		var tp = data.totalPage;
		var  $phpernotetable =  jQuery("#listTable5");
		 $phpernotetable.empty();
		str = '<tr class="header">  <td  >项目名称</td><td style="width:140px;" >当前利率</td> <td>投标奖励</td><td>借款总额</td><td>借款期限</td><td>进度</td><td>状态</td></tr>';
		var jsonRoot = data.jsonRoot;
		if(tp !=1&&tp !=0)
   	 ...
   	 document.getElementById("pageroot5").style.display="";
		}
		for(var i = 0; i < jsonRoot.length; i++) {
			str += '<tr><td style="text-align: left; "><a href="' + decodeURI(jsonRoot[i].actions, "utf-8") + '" >'+ jsonRoot[i].picture+ jsonRoot[i].title + "</a></td><td>"
				   + decodeURI(jsonRoot[i].rate, "utf-8") + "%</td><td>"
		 ...
		 $phpernotetable.html(str);
		 jQuery("#currentPage5").text("第"+cp+"页");
		 jQuery("#totalPage5").text("共"+tp+"页");
		 jQuery("#firstPage5").attr("href","javascript:getJSONData5(1,10)");
		 jQuery("#prevPage5").attr("href","javascript:getJSONData5("+(parseInt(cp)-1)+",10)");
		 jQuery("#nextPage5").attr("href","javascript:getJSONData5("+(parseInt(cp)+1)+",10)");
		 jQuery("#lastPage5").attr("href","javascript:getJSONData5("+(parseInt(tp))+",10)");
	});
}
 ...
 function getJSONDatas(currentPage,pageSize) {
    ...
    if(parseInt(jQuery("#endmoney").val()) < parseInt(jQuery("#startMoney").val())){
	 ...
	 if(document.getElementById("dayCheckId").checked){
			url = "tender/findlssuingBytender.action?currentPage="+currentPage+"&pageSize="+pageSize+"&returnWayId="+ jQuery("#time0").val()+"&moneyUseId="+ jQuery("#yongtu").val()+"&periodTimeId="
