...
<script type="text/javascript">
  function toMoneyPage(num){
	  ...
	  var mostMoney=$MP("#mostMoney").val();
   	  if(num==1){
			  ...
			  $MP("#page1-1").attr("href","<%=path %>/back/translate2.action?mark="+mark+"&page=1&startTime="+startTime+"&endTime="+endTime+"&dealUser="+dealUser+"&userName="+userName+"&state="+state+"&lessMoney="+lessMoney+"&mostMoney="+mostMoney); 
	  }
	  if(num==3){
		  if(page<pageNum){
			  $MP("#page1-3").attr("href","<%=path %>/back/translate2.action?mark="+mark+"&page="+(Number(page)+1)+"&startTime="+startTime+"&endTime="+endTime+"&dealUser="+dealUser+"&userName="+userName+"&state="+state+"&lessMoney="+lessMoney+"&mostMoney="+mostMoney);
		  }else{
			  $MP("#page1-3").attr("href","<%=path %>/back/translate2.action?mark="+mark+"&page="+(Number(page))+"&startTime="+startTime+"&endTime="+endTime+"&dealUser="+dealUser+"&userName="+userName+"&state="+state+"&lessMoney="+lessMoney+"&mostMoney="+mostMoney);
		  }
	  }
	  if(num==2){
		  if(page>1){
			  $MP("#page1-2").attr("href","<%=path %>/back/translate2.action?mark="+mark+"&page="+(Number(page)-1)+"&startTime="+startTime+"&endTime="+endTime+"&dealUser="+dealUser+"&userName="+userName+"&state="+state+"&lessMoney="+lessMoney+"&mostMoney="+mostMoney);
		  }else{
			  $MP("#page1-2").attr("href","<%=path %>/back/translate2.action?mark="+mark+"&page="+(Number(page))+"&startTime="+startTime+"&endTime="+endTime+"&dealUser="+dealUser+"&userName="+userName+"&state="+state+"&lessMoney="+lessMoney+"&mostMoney="+mostMoney);
		  }
	  }
	  if(num==4){
		  ...
		  if(p<1){
			  ...
			  $MP("#page1-4").attr("href","<%=path %>/back/translate2.action?mark="+mark+"&page="+Number(p)+"&startTime="+startTime+"&endTime="+endTime+"&dealUser="+dealUser+"&userName="+userName+"&state="+state+"&lessMoney="+lessMoney+"&mostMoney="+mostMoney);
	  }
	  if(num==5){
   		  	
			  $MP("#page1-5").attr("href","<%=path %>/back/translate2.action?mark="+mark+"&page="+Number(pageNum)+"&startTime="+startTime+"&endTime="+endTime+"&dealUser="+dealUser+"&userName="+userName+"&state="+state+"&lessMoney="+lessMoney+"&mostMoney="+mostMoney); 
	  }
	  
  }
  function search(){
	  var form=document.getElementById("search");
	  form.action="<%=path %>/back/translate2.action?mark=0";
	  form.submit();
  }
  function kai(id,userId){
  ...
  function commit(){
	  ...
	  var texplain=document.getElementById("texplain").value;
	  texplain=encodeURI(encodeURI(texplain));
	  form.action="<%=path %>/back/translate2.action?ok=1&texplain="+texplain+"&translateId="+hideId+"&userId="+userId;
	  form.submit();
      ...
      <dd>
          <div class="uew-select" style="float:left; line-height:34px;margin-left: 10px;"><div class="uew-select-value ue-state-default" style="width: 200px;"><em class="uew-icon uew-icon-triangle-1-s"></em></div> 
                 <select name="state" id="state" class="input" style="width: 210px;">
                  ...
                  <option value="1" <c:if test="${state=='1'}">selected="selected"</c:if>>审核通过，处理中</option> 
                 </select>
           </div> 
...
<!--         <li> <a href="#">将当前条件下数据导出为Excel</a></li> -->
    ...
    <table class="tablelist">
    	...
    	<tr>
        <th width="30px">ID</th>
        <th width="50px">会员名</th>
        <th width="50px">真实姓名</th>
        <th width="150px">银行信息</th>
        <th width="50px">提现金额</th>
        <th width="50px">提现手续费</th>
        <th width="50px">应到帐金额</th>
        <th width="100px">提现时间</th>
        <th width="50px">提现状态</th>
        <th width="50px">处理人</th>
        <th width="100px">处理时间</th>
        <th width="50px">处理说明</th>
        <th width="80px">操作</th>
        </tr>
        </thead>
        ...
        <thead>
        	...
        	<c:if test="${sessionScope.powerss[40]==1}">
        	<td>
        	<a class="green" href = "javascript:void(0)" onclick="kai(${translate.translateId},${translate.uservip.userId})">操作</a>
        	</td>
        	</c:if>
        	<c:if test="${sessionScope.powerss[40]!=1}"><td>--</td></c:if>
        	</tr>      	
        </thead>
        </c:forEach>
    </table>
       <form method="post" id="tip" onsubmit="getElementById('bt2').disabled=true;return true;" >
   <div id="hide" class="white_content" style="height: 280px;width: 480px">
	            <div class="light_title"><span>提示信息</span> <a href = "javascript:void(0)" onclick = "guan()" class="close"><img src="../images/close1.png" /></a></div>
	            <div class="letter_info">
	                 <p style="font-weight:normal; font-size:14px;"> <input type="radio" name="state1" id="state1" value="2"/>审核通过
	                  <input type="radio" name="state1" id="state3" value="3" checked="checked" />审核未通过</p>
	                  <div style="text-align:center;"><span style="line-height:30px; width:260px;">处理说明：</span> <textarea id="texplain" name="texplain1" style=" width:320px; height:65px; border:1px solid #ccc;">  </textarea></div>
		              <div class="tipbtn" style="margin-top:20px;margin-left:0;">
		                <input type="hidden" id="hideId"/>
		                <input type="hidden" id="userId"/>
				        <input type="button" value="确定" id="bt2" class="sure" onclick="commit()"/>
				        <input type="button" value="取消" class="cancel" onclick = "guan()" class="close">
		        	  </div>
	            </div>
