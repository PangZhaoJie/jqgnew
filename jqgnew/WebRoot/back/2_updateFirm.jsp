    	...
    	if(jQuery("#titleId").val()!=""&&
    	   ...
    	   jQuery("#manageMoney").val()!="" ){
    		if(jQuery("#periodtimeId").val()!="0" || jQuery("#perioddayId").val()!="0"){
    			...
    			form.action="<%=path %>/borrow/updateFirmLssuing?updateFlag=1&queryFlag="+queryFlag;
    			form.submit();
...
<body onload="tabTest(${mark})">
    <div class="rightinfo">
     <c:forEach var="lssuing" items="${lssuingList}">
       ...
       <div id="hotnews_content">
        ...
        <form  method="post" id="2_tabId1" action=""> 
           <ul class="m_forminfo">
           <li><label class="m_info_title">是否推荐：</label>  

              <label><input type="radio" name="lssuing.recommend" value="${lssuing.recommend}" id="RadioGroup1_0" <c:if test="${lssuing.isInvest==1}">checked</c:if> />  是</label>
              <label><input type="radio" name="lssuing.recommend" value="${lssuing.recommend}" id="RadioGroup1_1"  <c:if test="${lssuing.isInvest==0}">checked</c:if> /> 否</label>  
            </li>
             <li><label class="m_info_title">借款标题：</label>  
                 <input name="lssuing.lssuingId" type="hidden" class="dfinput" value="${lssuing.lssuingId}" readonly="readonly"id="titleId" size="37"/>
                 <input name="lssuing.title" type="text" class="dfinput" value="${lssuing.title}" readonly="readonly"id="titleId" size="37"/>
               <i>*借款标题不可修改</i>
            </li>
            ...
            <li><label class="m_info_title">借款人：</label>  
                ...
                <c:if test="${fs.userId==lssuing.uservip.userId }">
                	   <option value="${fs.userId }" selected="selected">${fs.userName}</option></c:if>	
                	<c:if test="${fs.userId!=lssuing.uservip.userId }">
                	   <option value="${fs.userId }">${fs.userName}</option></c:if>	
                </c:forEach> 
              </select>
               <i>*</i>
            </li>
            <li><label class="m_info_title">借款金额：</label>  
                <input name="lssuing.borrowMoney" value="${lssuing.borrowMoney }" onblur="count();"  onkeyup="value=value.replace(/[^\d\.]/g,'')" type="text" class="dfinput" id="borrowMoneyId" value="" size="37"/>
               <i>*</i>
            </li>
            <li>
            ...
            <select name="lssuing.moneymin.moneyMinId" id="moneyMinId" onchange="count();" class="input">
 	              	...
 	              	<c:forEach var="moneymin" items="${moneyminList}">
 	              	  <c:if test="${moneymin.moneyMinId==lssuing.moneymin.moneyMinId }">
                	   <option value="${moneymin.moneyMinId }" selected="selected">${moneymin.moneyMinName}</option></c:if>	
                	<c:if test="${moneymin.moneyMinId!=lssuing.moneymin.moneyMinId }">
		             <option  value="${moneymin.moneyMinId}">${moneymin.moneyMinName}元</option></c:if>	
					 </c:forEach> 
	              </select>  
            <i>*例如:50元</i>
            </li>
            ...
            <input name="lssuing.totalUnit"  type="text" class="dfinput" id="totalUnit" size="37" value="${lssuing.totalUnit}"/>
            <i>*如果计算错误可以手动修改</i>
            </li>
                ...
                <select name="lssuing.periodtime.periodTimeId" class="input"  id="periodtimeId">
	              	 <option value="0" selected="selected">-请选择月数-</option>              	 
 	              	 <c:forEach var="periodtime" items="${periodtimeList}">
 	              	   <c:if test="${periodtime.periodTimeId==lssuing.periodtime.periodTimeId }">
                	   <option value="${periodtime.periodTimeId }" selected="selected">${periodtime.periodTimeName}</option></c:if>	
                	  <c:if test="${periodtime.periodTimeId!=lssuing.periodtime.periodTimeId }">
...
<!-- 	              <input value="" id="perioddayListId"  type="hidden"/> -->
	              <select name="lssuing.periodday.periodDayId" class="input"  id="perioddayId" style="display:none">
	              	 <option value="0" selected="selected">-请选择天数-</option>              	 
 	              	 ...
 	              	 <c:if test="${periodday.periodDayId==lssuing.periodday.periodDayId }">
                	   <option value="${periodday.periodDayId }" selected="selected">${periodday.periodDayName}</option></c:if>	
                	 <c:if test="${periodday.periodDayId!=lssuing.periodday.periodDayId }">
            ...
            <input name="lssuing.award"  type="text" class="dfinput" id="award" value="${lssuing.award }" size="37"/>
            <i>%例如一个标借款期限6个月，你打算每月奖励0.01%，那么0.01*6=0.06，也就是应该在此处填写0.06</i>
            </li>
			 <li><label class="m_info_title">单人最大购买份数:</label>
			 <input name="lssuing.copies"  type="text" class="dfinput" id="copies" value="${lssuing.copies }" size="37"/><i>0表示无限制</i></li>
             <li><label class="m_info_title">借款管理费:</label>
             <input name="lssuing.manageMoney"  type="text" class="dfinput" id="manageMoney" value="${lssuing.manageMoney }" size="37"/><i>*</i></li>
         ...
         <div class="normal" id="list_2">
          <input  name="lssuing.lssuingComp.compId" id="compId" type="hidden" value="${lssuing.lssuingComp.compId}"/>
    	  <input   name="lssuing.lssuingComp.showImg" id="showImg" type="hidden" value="${lssuing.lssuingComp.showImg}"/>
          <ul class="m_forminfo">
			 ...
			 <div class="preview"  style="margin-left: 100px ;width: 1000px" id="tpxsId2">
              <div class="pre1" id="pre1${lssuing.lssuingComp.compId}">                   
                  <div class="pre1_t"><img src="<%=basePath%>${lssuing.lssuingComp.showImg}" /> 
                 <input value="删除" id="pic${lssuing.lssuingComp.compId}" style="margin-top: 10px;cursor: pointer;" onclick="deletePto3('${lssuing.lssuingComp.compId}')" type="button"/>
                 </div> 
            </div>
			 </div>
			 </li>
			 <li style="margin-top: 250px" id="lineTop">
			 <label class="m_info_title">借款方商业概述：</label>
			<textarea name="lssuing.lssuingComp.compInfo"   style="width:800px; height:100px; border:solid 1px #000000; border-radius:5px; resize:none;">${lssuing.lssuingComp.compInfo }</textarea>
			 </li>
			 <li>
			 <label class="m_info_title">借款方资产情况：</label>
			 <textarea name="lssuing.lssuingComp.compFund"  style="width:800px; height:100px; border:solid 1px #000000; border-radius:5px; resize:none;">${lssuing.lssuingComp.compFund }</textarea>
			 
			 </li>
			 <li>
			 <label class="m_info_title">借款方资金用途：</label>
			 <textarea name="lssuing.lssuingComp.borrowUse" style="width:800px; height:100px; border:solid 1px #000000; border-radius:5px; resize:none;">${lssuing.lssuingComp.borrowUse }</textarea>
			 </li>
			 <li>
			 <label class="m_info_title">风险控制措施：</label>
			 <textarea name="lssuing.lssuingComp.compWinCon" style="width:800px; height:100px; border:solid 1px #000000; border-radius:5px; resize:none;">${lssuing.lssuingComp.compWinCon }</textarea>
			 </li>
        ...
        <div class="normal" id="list_3">
         ...
         <ul class="m_forminfo">
            ...
            <div class="preview"  style="margin-left: 100px ;width: 1550px" id="tpxsId">
       		<c:forEach var="lssuingphoto" items="${lssuingphotoList}">
            <div class="pre1" id="pre1${lssuingphoto.photoId}">                   
                 <div class="pre1_t"><img src="<%=basePath%>${lssuingphoto.photo}" /> 
                 <input value="删除" id="pic${lssuingphoto.photoId}" style="margin-top: 10px;cursor: pointer;" onclick="deletePto('${lssuingphoto.photoId}')" type="button"/>
                 </div> 
            </div>
            <input id="imgUrlId" name="lssingphoto.photo" type="hidden" value="${lssuingphoto.photo}"/>
            </c:forEach>
