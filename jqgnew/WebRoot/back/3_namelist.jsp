...
function getJSONDatas(currentPage,pageSize) {
	...
	var url = "<%=path %>/overall/findcertification.action?currentPage="+currentPage+"&pageSize="+pageSize+"&ziduan=videoResult"+"&temp=" +Math.random();
	...
	$.getJSON(url,function(data){
		...
		var $table = $ ("#table");
		$table.empty();
		str = '<thead><tr> <th width="5%"  align="center" >ID</th><th width="10%"  align="center" >用户名</th><th width="10%"  align="center" >真实姓名</th><th width="20%"  align="center" >身份证号</th><th width="20%"  align="center" >身份证图片</th><th width="20%" align="center" >上传时间</th><th width="15%"  align="center" >操作</th></tr></thead>';
		var jsonRoot = data.jsonRoot;
		for(var i = 0; i < jsonRoot.length; i++) {
			str += '<tbody><tr><td>'+ jsonRoot[i].cid+ '<input type="hidden" value="'+jsonRoot[i].idnum+'" id="idnum"/></td><td>'
			...
			+	  ' <c:if test="${sessionScope.powerss[34]==1}"><a style="color:red;"href="javascript:void(0)" class="tablelink" onclick = "setUserId('+ jsonRoot[i].id+','+ jsonRoot[i].cid+  ');">审核</a></c:if><c:if test="${sessionScope.powerss[34]==0}"> --  </c:if> '
			    +"</td></tr>  </tbody>";
...
<li><a href="javascript:void(0);" onclick="parent.frames.topFrame.overall();">首页</a></li>
    ...
    <div>
    	<ul class="toolbar">
         <li ><span><img src="<%=path %>/back/images/t05.png" /></span>实名认证</li>
    ...
    <table class="tablelist" id="table">
        ...
        <th width="20%"  align="center" >身份证图片</th>
...
<input type="text" id="tpagesnone" style="display: none"/>
   ...
   <div id="light33" class="white_content">
       <div class="light_title">
          <span>审核结果</span> 
          <a href = "javascript:void(0)" onclick = "document.getElementById('light33').style.display='none';document.getElementById('fade').style.display='none';" class="close"><img src="images/close1.png"/></a>
      </div>
      <div class="light_box">
        ...
        <div class="tabcon" id="normalcon1">
         <div class="sublist">
             <form name="realname" id="realname" action="<%=path %>/overall/realname" method = "post" >
             <input id="radilValue" value="" type="hidden"/>
           <ul>
             <li>
               <label id="info_title">审核结果：</label>
                <p style="float:left; line-height:34px;">
  	                 <label> <input type="radio" name="radio" value="2" id="raidio1" checked="checked"/>通过</label>
                     <label> <input type="radio" name="radio" value="0" id="raidio2" /> 不通过</label>

               </p>
          </li>
          <li><input type="text" name="userId"  id="userId" style="display: none;"/><input type="text" name="quotaId"  id="cId" style="display: none;"/></li>
  ...
  <div id="pop" class="pop" style="display:none;margin-left:500px;margin-top:-20px;">
   ...
   getSex();
	...
	var radilValue = document.getElementById("radilValue").value;
	 ...
	 $m.ajax({
              		  ...
              		  success:function(data){
              		     ...
              		     if(result=="1"){
              		  	 ...
              		  	 if(result=="0"){
              		  		 document.getElementById("realname").submit();
							 alert("审核完成！");
              		  	 }
              		  }
              	  });
...
function getSex(){
 ...
 jQuery("#radilValue").val(value);
}

function setUserId(userId,cId){

input=  document.getElementById("userId");
    input.value=userId;
    input1=  document.getElementById("cId");
    input1.value=cId;
