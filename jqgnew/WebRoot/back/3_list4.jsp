...
function getJSONDatas(currentPage,pageSize) {
	...
	var url = "<%=path %>/overall/findUsersByvidio.action?currentPage="+currentPage+"&pageSize="+pageSize+"&ziduan=sceneResult"+"&temp=" +Math.random();
	...
	$.getJSON(url,function(data){
		...
		var $table = $ ("#table");
		$table.empty();
		str = '<thead><tr><th width="5%"  align="center" >ID</th><th width="6%"  align="center" >用户名</th><th width="6%"  align="center" >真实姓名</th><th width="6%"  align="center" >推荐人</th> <th width="8%"  align="center" >可用金额</th><th width="6%" align="center" >冻结金额</th><th width="6%" align="center" >待收金额</th> <th width="13%" align="center" >注册时间</th><th width="20%"  align="center" >操作</th></tr></thead>';
		...
		for(var i = 0; i < jsonRoot.length; i++) {
			str += '<tbody><tr><td>'+ jsonRoot[i].id+ '</td><td>'
				   ...
				   + decodeURI(jsonRoot[i].time, "utf-8") +"</td><td>"
					+	  ' <c:if test="${sessionScope.powerss[32]==1}"><a style="color:red;"href="javascript:void(0)" class="tablelink" onclick = "setUserId('+ jsonRoot[i].id+  ');">审核</a></c:if><c:if test="${sessionScope.powerss[32]!=1}"> --  </c:if> '  +"</td></tr>  </tbody>";
...
<li><a href="javascript:void(0);" onclick="parent.frames.topFrame.overall();">首页</a></li>
    ...
    <div>
    	<ul class="toolbar">
         <li ><span><img src="<%=path %>/back/images/t05.png" /></span>现场认证</li>
   ...
   <div id="light33" class="white_content">
      ...
      <div class="light_box">
        ...
        <div class="tabcon" id="normalcon1">
         <div class="sublist">
             <form name="vidio" id="vidio" action="<%=path %>/overall/scene" method = "post" >
           <ul>
             <li>
