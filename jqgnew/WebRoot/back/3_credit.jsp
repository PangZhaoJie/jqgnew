...
function btn(){
   ...
   // var uPwdRE=new RegExp("^(+-)?(([1-9]{1}\d*)|([0]{1}))(\.(\d){1,2})?$");
	if (!uPwdRE.test(money.value)) {
	    alert("请输入正确额度 ！");
	    money.focus();
        return;
	}
	document.getElementById("uptcredit").submit();
...
<li><a href="javascript:void(0);" onclick="parent.frames.topFrame.overall();">首页</a></li>
    ...
    <div class="rightinfo">
       ...
       <div id="hotnews_caption">
            <ul id="h_menu">                  
                 <li class="current" onclick="secBoard('hotnews_caption','list',1);">调整信用额度</li>
             </ul>
         </div> 
    
       <div id="hotnews_content">
          <div class="current" id="list_1">
           <form name="uptcredit" id="uptcredit" action="<%=path %>/overall/uptcredit" method = "post" >
             <ul class="m_forminfo">
                ...
                <li><label class="m_info_title">信用额度：</label><input name="amoney" id="money" type="text" class="dfinput" value="1000" size="37"/><i>信用额度最大位数为10位，保留小数点后两位；如果是减少余额，请填负数，如'-1000'</i></li>
