...
<title>${latestnewsdetail.title}</title>
<meta name="Keywords" content="${latestnewsdetail.importWord}" />
<meta  name="descripton" content="${latestnewsdetail.synopsis}" />
...
<div class="mem10">
    ...
    <c:forEach  var="frontmenus" items="${frontmenus}">	
				     <li><a  class="<c:if test="${frontmenus.frontMenuCode==latestnewsdetail.frontmenu.frontMenuCode}">active</c:if>" href="<%=path%>${frontmenus.url}">${frontmenus.name}</a></li>
			   </c:forEach>  
		 <div style="clear:both;"></div>
        </ul>    
        <div class="mem10_content">
        <!-- 文章单页 -->
        <div class="article_title"><h1>${latestnewsdetail.title}</h1></div>
        <div class="news_dh">发表时间：<fmt:formatDate value="${latestnewsdetail.publishTime}" pattern="yyyy-MM-dd  HH:mm:ss" /></div>
                           
        <div class="article_content">
        ${latestnewsdetail.content}
        </div>
        <div class="pagecontent">
               
           
               <s:if test="#latestnewsOnPage==null">
               上一篇：没了
               </s:if>
               <s:else>
                上一篇：<a href="<%=path %>/news/newdetail?newsId=${latestnewsOnPage.newsId}">${latestnewsOnPage.title}</a> 
               </s:else>
               <s:if test="#latestnewsDownPage==null">
               <br />下一篇：没了
               </s:if>
               <s:else>
               <br /> 下一篇：<a href="<%=path%>/news/newdetail?newsId=${latestnewsDownPage.newsId}">${latestnewsDownPage.title}</a> 
               </s:else>
              </div> 
            </div>
</div>
